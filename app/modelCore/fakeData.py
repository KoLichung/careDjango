...
def seedData():
    ...
    Service.objects.create(name="安全維護",remark="預防跌倒、陪同散步、推輪椅、心靈陪伴",is_increase_price=True)
    Service.objects.create(name="協助進食",remark="用餐，按醫囑給藥",is_increase_price=True)
    Service.objects.create(name="協助如廁",remark="大小便處理、更換尿布、會陰沖洗",is_increase_price=True)
    Service.objects.create(name="身體清潔",remark="沐浴、擦澡",is_increase_price=True)
    Service.objects.create(name="陪同就醫",remark="陪伴看診、洗腎，代領藥品",is_increase_price=True)
    Service.objects.create(name="陪同復健",is_increase_price=True)
    Service.objects.create(name="代購物品",remark="代買生活必需品以有發票或收據為主；代購物品期間服務者無法負責安全維護，家屬需自行評估被照顧者狀況",is_increase_price=True)
    Service.objects.create(name="簡易備餐",remark="依現有食材簡易煮粥、麵食或加熱即食品；僅提供被照顧者與一位家屬餐食",is_increase_price=True)
    Service.objects.create(name="家務協助",remark="簡易掃地拖地，清洗衣物、床單",is_increase_price=True)
    Service.objects.create(name="鼻胃管灌食",is_increase_price=True)
    Service.objects.create(name="管路清潔",is_increase_price=True)
    Service.objects.create(name="翻身拍背",remark="僅提供長期臥床或手術無法自行翻身的病人",is_increase_price=True)
    Service.objects.create(name="被動關節運動",remark="僅提供長期臥床、癱瘓、昏迷或關節炎的病人； 病人有骨質疏鬆症、骨折病史請告知",is_increase_price=True)
