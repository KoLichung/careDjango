...
def importCityCounty():
    ...
    for index, row in enumerate(reader):
        if index != 0:
            ...
            if County.objects.filter(city=city, name=county_name).count()==0:
                county = County()
            else:
...
def seedData():
    ...
    License.objects.create(name="良民證-警察刑事紀錄證明書",remark="若未提供，服務者頁面會顯示「未提供」供預訂者參考")
    ...
    License.objects.create(name="高中(職)以上護理、照顧相關科系(組)畢業證書")
    ...
    Service.objects.create(name="管路安全照護")
